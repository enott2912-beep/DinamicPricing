...
def get_recommendations_all_products(prod_df: pd.DataFrame, work_df: pd.DataFrame, include_lightgbm: bool = True) -> dict:
    ...
    for p in sku_list:
        ...
        rec_prices_rules.append(float(np.mean(rule_prices)) if rule_prices else 0.0)
        opts.append(float(np.mean(reg_prices)) if reg_prices else 0.0)
        rel_flags.append(all(reg_rel_entity) if reg_rel_entity else False)

        rule_name = rule_names[0] if len(set(rule_names)) == 1 else f"mixed ({len(set(rule_names))})"
        growth_rules_pct = ((pred_rules_sum - cur_profit_sum) / cur_profit_sum * 100) if cur_profit_sum > 0 else 0.0
        growth_reg_pct = ((pred_reg_sum - cur_profit_sum) / cur_profit_sum * 100) if cur_profit_sum > 0 else 0.0
        growth_nl_pct = ((pred_nl_sum - cur_profit_sum) / cur_profit_sum * 100) if cur_profit_sum > 0 else 0.0

        rule_rows.append({
            "Товар": p,
            "Цена, ₽": round(float(np.mean(last_prices)), 2),
            "Цена (правила), ₽": round(float(np.mean(rule_prices)) if rule_prices else 0.0, 2),
            "Правило": rule_name,
        ...
        reg_rows.append({
            "Товар": p,
            "Цена, ₽": round(float(np.mean(last_prices)), 2),
            "Цена (регр.), ₽": round(float(np.mean(reg_prices)) if reg_prices else 0.0, 2),
            "Надёжн.": "да" if all(reg_rel_entity) and reg_rel_entity else "нет",
            "Прогноз прибыли, ₽": round(pred_reg_sum, 2),
            "Δ к факту, %": round(growth_reg_pct, 1),
            "A": round(float(np.mean(reg_a_vals)) if reg_a_vals else 0.0, 2),
            "B": round(float(np.mean(reg_b_vals)) if reg_b_vals else 0.0, 4),
        })

        if include_lightgbm:
            total_pred_nl += pred_nl_sum
            nl_opts.append(float(np.mean(nl_prices)) if nl_prices else 0.0)
            nl_rel_flags.append(all(nl_rel_entity) if nl_rel_entity else False)
            warn_unique = list(dict.fromkeys([w for w in nl_warns if w]))
            nl_rows.append({
                "Товар": p,
                "Цена, ₽": round(float(np.mean(last_prices)), 2),
                "Цена (LightGBM), ₽": round(float(np.mean(nl_prices)) if nl_prices else 0.0, 2),
                "Надёжн.": "да" if all(nl_rel_entity) and nl_rel_entity else "нет",
...
def get_recommendations_single_product(
    ...
    rec_price_rules = float(np.mean(rule_prices)) if rule_prices else float(np.mean(last_prices))
    opt_price_reg = float(np.mean(reg_prices)) if reg_prices else float(np.mean(last_prices))
    opt_price_nl = float(np.mean(nl_prices)) if nl_prices else float(np.mean(last_prices))
    growth_rules = ((pred_rules_sum - current_profit_sum) / current_profit_sum * 100) if current_profit_sum > 0 else 0.0
    growth_reg = ((pred_reg_sum - current_profit_sum) / current_profit_sum * 100) if current_profit_sum > 0 else 0.0
    growth_nl = ((pred_nl_sum - current_profit_sum) / current_profit_sum * 100) if current_profit_sum > 0 else 0.0
    last_row = pd.Series({
    ...
    return {
        ...
        "a": float(np.mean(reg_a_vals)) if reg_a_vals else 0.0,
        "b": float(np.mean(reg_b_vals)) if reg_b_vals else 0.0,
        "opt_price_reg": opt_price_reg,
        "is_reg_reliable": all(reg_rel_flags) if reg_rel_flags else False,
        "fc_reg": {
            "forecast_profit": round(pred_reg_sum, 2),
            "growth_pct": round(growth_reg, 1),
        },
        "opt_price_nl": opt_price_nl,
        "is_nl_reliable": all(nl_rel_flags) if nl_rel_flags else False,
